...
from VT_Patcher.Utils.Files_Check import __version__


Tag = f"\n{C.CC}————|———————|————{C.G}•❀ {C.OG}Tag {C.G}❀•{C.CC}————|———————|————\n"

FE = f"{C.P}\n   |\n   ╰{C.CC}┈{C.OG}➢ {C.G}VT_Patcher"
...
class CustomArgumentParser(M.argparse.ArgumentParser):
    ...
    def print_help(self):
        ...
        print(f"\n{C.INFO} VT_Patcher Default Patch is VPN & SSL Bypass, Show Other Patch Flags List with: {C.G}VT_Patcher -O{C.C}\n")
...
def parse_arguments():
    ...
    if any(arg.startswith('-') for arg in args):
        parser = CustomArgumentParser(description=f'{C.C}VT_Patcher v{__version__}')
    else:
